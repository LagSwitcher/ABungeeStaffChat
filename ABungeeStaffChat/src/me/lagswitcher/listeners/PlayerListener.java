  ...
  public void onPlayerChat(ChatEvent e)
  {
    ...
    if ((pp.hasPermission("sc.toggle")) || (pp.hasPermission("sc.*")))
    {
      ...
      if (player.isScToggled())
      {
        ...
        BungeeStaffChat.getInstance().getProxy().getPluginManager().dispatchCommand(pp, BungeeStaffChat.getInstance().getConfig().getString("sc-command").replaceAll("/", "") + " " + e.getMessage());
    ...
    if (BungeeStaffChat.getInstance().isShortcutEnabled()) {
      if ((pp.hasPermission("sc.shortcut")) || (pp.hasPermission("sc.*")))
      {
        ...
        if (e.getMessage().toCharArray()[0] == BungeeStaffChat.getInstance().getShortcut())
        {
          ...
          BungeeStaffChat.getInstance().getProxy().getPluginManager().dispatchCommand(pp, BungeeStaffChat.getInstance().getConfig().getString("sc-command").replaceAll("/", "") + " " + e.getMessage().substring(1, e.getMessage().length()));
        }
      }
    }
  }
